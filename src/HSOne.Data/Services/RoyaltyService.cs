...
namespace HSOne.Data.Services
{
    public class RoyaltyService : IRoyaltyService
    {
        ...
        public async Task<List<RoyaltyReportByMonthDto>> GetRoyaltyReportByMonthAsync(Guid? userId, int fromMonth, int fromYear, int toMonth, int toYear)
        {
            using (var conn = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                var coreSql = @"select 
                                    ...
                                    sum(case when p.Status = 1 then 1 else 0 end) as NumberOfDraftPosts,
                                    ...
                                    sum(case when p.Status = 4 then 1 else 0 end) as NumberOfPublishPosts,
                                    sum(case when p.Status = 4 and p.IsPaid = 1 then 1 else 0 end) as NumberOfPaidPublishPosts,
                                    sum(case when p.Status = 4 and p.IsPaid = 0 then 1 else 0 end) as NumberOfUnpaidPublishPosts
                                from Posts p
                                group by 
        ...
        public async Task PayRoyaltyForUserAsync(Guid fromUserId, Guid toUserId)
        {
            ...
            var unpaidPublishPosts = await _unitOfWork.Posts.GetUnpaidPublishPostsAsync(toUserId);
            ...
            _unitOfWork.Transactions.Add(new Transaction()
            {
                ...
                Note = $"{fromUser.UserName} pays royalties to {toUser.UserName}"
            });
